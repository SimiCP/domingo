...
// value will be calculated to a new amount(currency) value.
// The program will create an output file milesamount.txt with the calculated
// reimbursement amounts for each mile value processed, the output is displayed
...
{		// main method
	...
	{ 
    	double miles;   	 // declaration of miles variable
      ...
      {    	
       	...
       	miles = inFile.nextDouble();
				...
				// if statment to calculate all mileage values from input file above zero
				// each value will pass through the if and else if statements and will be calculated
				// into a new (amount) value to correspond to the according mileage value
        	if (miles > 0)						
        ...
        { 	
		  		if (miles == 0) 					
					counter++;		// adds one more to count for values equal to zero
				amount = 0; 						
